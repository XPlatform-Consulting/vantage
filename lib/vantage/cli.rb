...
module Vantage
  ...
  module CLI
    ...
    class CommonOptionParser < ::OptionParser
      ...
      def initial_arguments(ia = ARGV)
        @initial_arguments ||= ia.dup
      end
      ...
      def parse_common(command_line_arguments = ARGV, options = { })
        parse_common!(command_line_arguments.dup, options)
      end
...
@cli_class ||= Vantage::CLI
def cli_class; @cli_class ||= Vantage::CLI end
...
def add_common_options(option_parser = common_option_parser, _options = (options || { }))
  ...
  option_parser.on('--log-level LEVEL', Vantage::CLI::LOGGING_LEVELS.keys, "Logging level. Available Options: #{Vantage::CLI::LOGGING_LEVELS.keys.join(', ')}",
                   "\tdefault: #{Vantage::CLI::LOGGING_LEVELS.invert[_options[:log_level]]}") { |v| _options[:log_level] = Vantage::CLI::LOGGING_LEVELS[v] }
